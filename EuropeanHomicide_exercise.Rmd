...
```{r plot-homicides split into countries}
Western_Europe %>% 
  ggplot(aes(x=Year, y=homicide_per_100k, color = country))+
  geom_line()+
  facet_wrap(~country,
...
Answer: According to the graph of the duration of danish kings' rule there's a steady rise in the number of years ruling, especially after 1400. In the graph over homicides in Scandinavia there's a drop in homicides after 1400, so the rise in years ruling might correlate with the reduced possibility of getting murdered. 

```{r kings duration of rule, eval=TRUE}
read_csv2("data/danish_kings_4 copy.csv") -> kings
kings <- kings[1:55, 1:5]

kings %>% 
  mutate(duration = Rule_end - Rule_start) -> kings

kings %>% 
  mutate(midyear = Rule_end - (Rule_end - Rule_start)/2) -> kings

kings%>%
  ggplot(aes(x = midyear, y = duration))+
  geom_smooth(color = "orange")+
  labs(title = "Duration of danish kings' rule", 
       x = "Middle year of reign", 
       y = "number of years ruling")+
  theme_bw()
```
...
When looking at the data visualisations, it is clear that the homicide rate have dropped significantly since the 13th century in all the above mentioned countries. The countries have very different homicide rates up until the 15th century. Italy's homicide rate is for example higher than the other countries. The homicide rates are very close to each other in all of the countries after the 19th century, except for Italy which is still higher. The rates have dropped even more after the 21st century and the countries have almost the same homicide rate. So, based on these visualisations, the conclusion would be that we are more civilized today. 
